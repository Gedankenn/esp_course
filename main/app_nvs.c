...
const char app_nvs_sta_creds_name_space[] = "stacreds";
...
esp_err_t app_nvs_save_sta_creds(void)
{
    ...
    if(wifi_sta_config)
    {
        esp_err = nvs_open(app_nvs_sta_creds_name_space, NVS_READWRITE, &handle);
        if (esp_err != ESP_OK)
        {
        ...
        // Set SSID
        esp_err = nvs_set_blob(handle, "ssid", wifi_sta_config->sta.ssid, MAX_SSID_LENGHT);
        if (esp_err != ESP_OK)
        {
            printf("app_nvs_save_sta_creds: Error (%s) setting SSID to NVS!\n", esp_err_to_name(esp_err));
            return esp_err;
        }
        esp_err = nvs_set_blob(handle, "password", wifi_sta_config->sta.password, MAX_PASSWORD_LENGHT);
        if (esp_err != ESP_OK)
        {
        ...
        nvs_close(handle);
        ESP_LOGI(TAG, "app_nvs_save_sta_creds: wrote wifi sta config: Station SSID: %s Passwod: %s",wifi_sta_config->sta.ssid,wifi_sta_config->sta.password);
    }
...
bool app_nvs_load_sta_creds(void)
{
    ...
    if(nvs_open(app_nvs_sta_creds_name_space, NVS_READONLY, &handle) == ESP_OK)
    {
        wifi_config_t *wifi_sta_config = wifi_app_get_wifi_config();
...
esp_err_t app_nvs_clear_sta_creds(void)
{
    nvs_handle handle;
    esp_err_t esp_err;
    ESP_LOGI(TAG, "app_nvs_clear_sta_creds: Clearinf wifi station mode credentials from flash");
    esp_err = nvs_open(app_nvs_sta_creds_name_space,NVS_READWRITE, &handle);
    if(esp_err != ESP_OK)
    {
