 ...
 * @param event_base the base id of the vent to register the handler for
 * @param event_id for th event to register the handler for
...
/**
 * configure the wifi acess point settings and assings the static ip to softAP
*/
static void wifi_app_soft_ap_config(void)
{
    ...
    inet_pton(AF_INET, WIFI_AP_IP, &ap_ip_info.ip);         // assing acces point's static IP, GW, and netmask
    ...
    ESP_ERROR_CHECK(esp_wifi_set_bandwidth(ESP_IF_WIFI_AP, WIFI_AP_BAMDWIDTH));
    ESP_ERROR_CHECK(esp_wifi_set_ps(WIFI_STA_POWER_SAVE));                      // Power save set  to null
}

/**
 * Initialize the wifi application event handler for the wifi and ip events
*/
static void wifi_app_event_handler_init(void)
{
    // Evento loop for the wifi_driver
        ...
        if (xQueueReceive(wifi_app_queue_handle, &msg, portMAX_DELAY))
        {
            switch(msg.msgID)
            {
                ...
                case WIFI_APP_MSG_STA_CONNECTED_GOT_IP:
                    ...
                    {
                        xEventGroupClearBits(wifi_app_event_group, WIFI_APP_CONNECTING_USING_SAVED_CREDS_BIT); ///> Clear the bits, in case we want to disconnect and reconnect
                    }
                    else
                    {
                ...
                case WIFI_APP_MSG_USER_REQUESTED_STA_DISCONNECT:
                    ...
                    if(eventBits & WIFI_APP_STA_CONNECTED_GOT_IP_BIT)
                    {
                        ...
                        g_retry = MAX_CONNECTION_RETRIES;
                ...
                case WIFI_APP_MSG_STA_DISCONNECTED:
                    ...
                    {
                        ESP_LOGI(TAG, "WIFI_APP_MSG_STA_DISCONNECT: USER REQUSTED DISCONNECT");
                    ...
                    {
                        ESP_LOGI(TAG, "WIFI_APP_MSG_STA_DISCONNECT: ATTEMPT FAILED, CHECK WIFI ACCESS POINT AVALIABILITY");
